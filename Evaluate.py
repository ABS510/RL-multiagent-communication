...
def evaluate(env, agents, models, game_nums, maximum_frame, logger, epsilon=0.05):
    ...
    for game_num in range(game_nums):
        ...
        for agent in agents:
            ...
            logger.info(f"Evaluation: Agent {agent} Intention Metrics: {intention_metrics}")
    ...
    for agent in agents:
        ...
        logger.info(f"Evaluation: Agent {agent} mean intention followed: {np.mean(intentions_followed)}")
        logger.info(f"Evaluation: Agent {agent} intention followed std: {np.std(intentions_followed)}")
        logger.info(f"Evaluation: Agent {agent} mean intention not followed: {np.mean(intentions_not_followed)}")
        logger.info(f"Evaluation: Agent {agent} intention not followed std: {np.std(intentions_not_followed)}")
        logger.info(f"Evaluation: Agent {agent} mean no-intentions: {np.mean(no_intentions)}")
        logger.info(f"Evaluation: Agent {agent} no-intentions std: {np.std(no_intentions)}")